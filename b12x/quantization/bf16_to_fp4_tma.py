...
import cutlass, cutlass.cute as cute, cutlass.pipeline as pipeline
...
def rcp_rn_f32(a, *, loc=None, ip=None):
    return Float32(llvm.inline_asm(T.f32(), [Float32(a).ir_value(loc=loc, ip=ip)],
        "{\n.reg .f32 one;\nmov.f32 one, 0f3F800000;\ndiv.rn.f32 $0, one, $1;\n}",
        "=f,f", has_side_effects=False, is_align_stack=False, asm_dialect=llvm.AsmDialect.AD_ATT))
@dsl_user_op
def ld_global_bf16x2(tensor, coord, *, loc=None, ip=None):
    ...
    return Uint32(llvm.inline_asm(
        T.i32(), [address], "ld.global.b32 $0, [$1];", "=r,l",
...
def cvt_bf16x2_to_f32x2(packed, *, loc=None, ip=None):
    result = llvm.inline_asm(
        ...
        "=f,=f,r", has_side_effects=False, is_align_stack=False,
        asm_dialect=llvm.AsmDialect.AD_ATT, loc=loc, ip=ip,
    )
    return (
...
def cvt_bf16x2_to_f32x2_scaled(packed, scale, *, loc=None, ip=None):
    result = llvm.inline_asm(
        ...
        "mul.rn.f32 $1, fhi, $3; }",
        "=f,=f,r,f", has_side_effects=False, is_align_stack=False,
        asm_dialect=llvm.AsmDialect.AD_ATT, loc=loc, ip=ip,
    )
    return (
        Float32(llvm.extractvalue(T.f32(), result, [0], loc=loc, ip=ip)),
...
@dsl_user_op
def fp8_e4m3_to_f32_and_effective_rcp(fp8_val, global_scale_recip, *, loc=None, ip=None):
    return Float32(llvm.inline_asm(T.f32(), [
        ...
        "=f,r,f", has_side_effects=False, is_align_stack=False, asm_dialect=llvm.AsmDialect.AD_ATT))
@dsl_user_op
...
class TestKernel:
    ...
    def __call__(self, bf16_input: cute.Tensor, global_scale: cute.Tensor,
                 packed_a: cute.Tensor, scale_storage: cute.Tensor,
        ...
        self.kernel(
            bf16_input, tma_store_a, gOA,
            global_scale, scale_storage, fp4_staged,
    ...
    def kernel(self,
               ...
               global_scale: cute.Tensor, scale_storage: cute.Tensor,
               fp4_smem: cute.ComposedLayout, fp4_cs: cutlass.Constexpr):
        ...
        class S:
            ...
            sGlobalScaleRecip: cute.struct.Align[cute.struct.MemRange[cutlass.Float32, 1], 4]
        st = smem.allocate(S)
        ...
        while tile_idx < total_tiles:
            ...
            while blk < Int32(128 * 8):
                ...
                effective_recip, sbyte = quantize_scale_fp4_fast(
                    bmax, block_global_scale, global_scale_recip,
                )
                p64 = Uint64(0)
