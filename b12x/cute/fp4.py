...
def relu2_quantize_grouped_nvfp4_torch(
    ...
    """ReLU²(x) + grouped NVFP4 quantize.  Non-gated (no gate/up split)."""
...
def relu2_16(x: cute.Tensor) -> cute.Tensor:
    """ReLU²(x) for 16 float32 elements.  Non-gated — no up tensor."""
    out = cute.make_rmem_tensor((16,), Float32)
    for i in cutlass.range_constexpr(16):
...
def relu2_quantize_block_fp4(
    ...
    """Fused ReLU²(x) + FP4 quantize for 16 elements (non-gated)."""
