...
COMPRESSED_MLA_NOPE_ROPE_BYTES_PER_TOKEN = COMPRESSED_MLA_NOPE_DIM + COMPRESSED_MLA_ROPE_DIM * 2
COMPRESSED_MLA_SCALE_BYTES_PER_TOKEN = 8
COMPRESSED_MLA_BYTES_PER_TOKEN = COMPRESSED_MLA_NOPE_ROPE_BYTES_PER_TOKEN + COMPRESSED_MLA_SCALE_BYTES_PER_TOKEN
...
def compressed_mla_page_nbytes(page_size: int) -> int:
    ...
    return math.ceil(unpadded / COMPRESSED_MLA_NOPE_ROPE_BYTES_PER_TOKEN) * COMPRESSED_MLA_NOPE_ROPE_BYTES_PER_TOKEN
...
def _float_to_ue8m0_scale(max_abs: torch.Tensor) -> torch.Tensor:
    safe = torch.where(max_abs > 0, max_abs / COMPRESSED_MLA_FP8_MAX, torch.ones_like(max_abs))
    exponent = torch.ceil(torch.log2(safe)).to(torch.int32)
    exponent = torch.clamp(exponent, -COMPRESSED_MLA_UE8M0_BIAS, 255 - COMPRESSED_MLA_UE8M0_BIAS)
...
def quantize_compressed_mla_nope_reference(k_nope: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    """Quantize noPE rows into E4M3 FP8 plus seven UE8M0 scale bytes."""

    if k_nope.shape[-1] != COMPRESSED_MLA_NOPE_DIM:
        raise ValueError(f"k_nope last dim must be {COMPRESSED_MLA_NOPE_DIM}, got {k_nope.shape[-1]}")
    grouped = k_nope.float().reshape(*k_nope.shape[:-1], COMPRESSED_MLA_NOPE_GROUPS, COMPRESSED_MLA_NOPE_GROUP_SIZE)
    max_abs = grouped.abs().amax(dim=-1)
    scale_ue8m0 = _float_to_ue8m0_scale(max_abs)
    scale = ue8m0_to_float(scale_ue8m0).unsqueeze(-1)
    scaled = torch.clamp(grouped / scale, -COMPRESSED_MLA_FP8_MAX, COMPRESSED_MLA_FP8_MAX)
...
def pack_compressed_mla_kv_cache_reference(
    ...
    *,
    ...
    if k_nope.ndim != 2 or k_nope.shape[-1] != COMPRESSED_MLA_NOPE_DIM:
        raise ValueError(f"k_nope must have shape [tokens, {COMPRESSED_MLA_NOPE_DIM}], got {tuple(k_nope.shape)}")
    if k_rope.ndim != 2 or k_rope.shape[-1] != COMPRESSED_MLA_ROPE_DIM:
        raise ValueError(f"k_rope must have shape [tokens, {COMPRESSED_MLA_ROPE_DIM}], got {tuple(k_rope.shape)}")
    ...
    if num_pages < min_pages:
        raise ValueError(f"num_pages {num_pages} cannot hold {n_tokens} tokens at page_size {page_size}")

    page_nbytes = compressed_mla_page_nbytes(page_size)
    scale_offset = compressed_mla_scale_region_offset(page_size)
    cache = torch.zeros((num_pages, page_nbytes), dtype=torch.uint8, device=k_nope.device)
    if n_tokens == 0:
        return cache
    ...
    payload[pages, token_offsets, :COMPRESSED_MLA_NOPE_DIM] = k_nope_fp8.view(torch.uint8).view(n_tokens, COMPRESSED_MLA_NOPE_DIM)
    payload[pages, token_offsets, COMPRESSED_MLA_NOPE_DIM:COMPRESSED_MLA_NOPE_ROPE_BYTES_PER_TOKEN] = (
        k_rope.to(torch.bfloat16).view(torch.uint8).view(n_tokens, COMPRESSED_MLA_ROPE_DIM * 2)
    )
    scales[pages, token_offsets, :COMPRESSED_MLA_NOPE_GROUPS] = scale_ue8m0.reshape(n_tokens, COMPRESSED_MLA_NOPE_GROUPS)
    return cache
...
def unpack_compressed_mla_kv_cache_reference(
    ...
    *,
    ...
    if n_tokens == 0:
        empty_nope = torch.empty((0, COMPRESSED_MLA_NOPE_DIM), dtype=torch.float32, device=kv_cache.device)
        empty_rope = torch.empty((0, COMPRESSED_MLA_ROPE_DIM), dtype=torch.float32, device=kv_cache.device)
        return empty_nope, empty_rope

    indices = torch.arange(n_tokens, dtype=torch.int64, device=kv_cache.device)
    return gather_compressed_mla_kv_cache_reference(kv_cache, indices, page_size=page_size)
...
def gather_compressed_mla_kv_cache_reference(
    ...
    *,
    ...
    if indices.numel() == 0:
        empty = torch.empty((0, COMPRESSED_MLA_HEAD_DIM), dtype=torch.float32, device=kv_cache.device)
        return empty, empty
    ...
    token_base = pages * page_nbytes + token_offsets * COMPRESSED_MLA_NOPE_ROPE_BYTES_PER_TOKEN
    nope_offsets = token_base[:, None] + torch.arange(COMPRESSED_MLA_NOPE_DIM, device=kv_cache.device, dtype=torch.int64)
    rope_byte_offsets = token_base[:, None] + COMPRESSED_MLA_NOPE_DIM + torch.arange(
    ...
    scale_offsets = (
        ...
        + torch.arange(COMPRESSED_MLA_NOPE_GROUPS, device=kv_cache.device, dtype=torch.int64)
    )

    nope_fp8 = flat_cache[nope_offsets].contiguous().view(torch.float8_e4m3fn).view(-1, COMPRESSED_MLA_NOPE_DIM)
    scale = ue8m0_to_float(flat_cache[scale_offsets]).view(-1, COMPRESSED_MLA_NOPE_GROUPS, 1)
    nope = (nope_fp8.to(torch.float32).view(-1, COMPRESSED_MLA_NOPE_GROUPS, COMPRESSED_MLA_NOPE_GROUP_SIZE) * scale).reshape(
        -1, COMPRESSED_MLA_NOPE_DIM
    )
    rope = flat_cache[rope_byte_offsets].contiguous().view(torch.bfloat16).view(-1, COMPRESSED_MLA_ROPE_DIM).to(torch.float32)
...
def compressed_sparse_mla_reference(
    ...
    *,
    ...
    if swa_topk_lengths.shape != (rows,):
        raise ValueError(f"swa_topk_lengths must have shape [{rows}], got {tuple(swa_topk_lengths.shape)}")
    _validate_flat_cache(swa_k_cache, page_size=swa_page_size)

    has_extra = extra_k_cache is not None or extra_indices is not None or extra_topk_lengths is not None
    if has_extra:
        if extra_k_cache is None or extra_indices is None or extra_topk_lengths is None:
            raise ValueError("extra_k_cache, extra_indices, and extra_topk_lengths must be provided together")
        if extra_page_size is None:
            raise ValueError("extra_page_size is required when extra_k_cache is provided")
        ...
        if extra_topk_lengths.shape != (rows,):
            raise ValueError(f"extra_topk_lengths must have shape [{rows}], got {tuple(extra_topk_lengths.shape)}")
    ...
    out = torch.empty((rows, heads, COMPRESSED_MLA_HEAD_DIM), dtype=torch.float32, device=q3.device)
    lse = torch.empty((rows, heads), dtype=torch.float32, device=q3.device)
    q_f32 = q3.float()

    for row in range(rows):
        swa_len = _valid_prefix_length(swa_topk_lengths[row], swa_indices_2d[row], swa_indices_2d.shape[1])
        if has_extra and extra_indices_2d is not None and extra_topk_lengths is not None:
            extra_len = _valid_prefix_length(extra_topk_lengths[row], extra_indices_2d[row], extra_indices_2d.shape[1])
            if extra_len:
                ...
                extra_k, extra_v = gather_compressed_mla_kv_cache_reference(
                    extra_k_cache, extra_indices_2d[row, :extra_len], page_size=extra_page_size
                )
            else:
                extra_k = torch.empty((0, COMPRESSED_MLA_HEAD_DIM), dtype=torch.float32, device=q3.device)
                extra_v = extra_k
        else:
            extra_len = 0
            extra_k = torch.empty((0, COMPRESSED_MLA_HEAD_DIM), dtype=torch.float32, device=q3.device)
        ...
        if swa_len:
            swa_k, swa_v = gather_compressed_mla_kv_cache_reference(swa_k_cache, swa_indices_2d[row, :swa_len], page_size=swa_page_size)
        else:
            swa_k = torch.empty((0, COMPRESSED_MLA_HEAD_DIM), dtype=torch.float32, device=q3.device)
            swa_v = swa_k
...
def _normalize_q(q: torch.Tensor) -> torch.Tensor:
    ...
    if q.ndim != 3 or q.shape[-1] != COMPRESSED_MLA_HEAD_DIM:
        raise ValueError(f"q must have shape [rows, heads, {COMPRESSED_MLA_HEAD_DIM}], got {tuple(q.shape)}")
    return q
...
def _validate_flat_cache(kv_cache: torch.Tensor, *, page_size: int) -> None:
    if kv_cache.ndim != 2:
        raise ValueError(f"kv_cache must be a flat [pages, page_nbytes] uint8 buffer, got {tuple(kv_cache.shape)}")
    ...
    if kv_cache.shape[1] != expected:
        raise ValueError(f"kv_cache page byte width must be {expected} for page_size {page_size}, got {kv_cache.shape[1]}")
