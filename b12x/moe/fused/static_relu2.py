"""
MoEStaticKernelRelu2 — static-scheduled routed NVFP4 MoE kernel for SM120 (Blackwell).

ReLU2 (non-gated) variant of MoEStaticKernel.  Forked because CuTe DSL
cannot handle conditional control flow in kernel code — separate classes
are the only viable approach.

Differences from the SiLU/SwiGLU (gated) variant:
  - Single FC1 GEMM (no up pass): w1 is [E, I, K//2] not [E, 2*I, K//2]
  - Activation: relu(fc1_out)^2 instead of SiLU(gate) * up
  - No up pipeline, no up shared memory buffers, no gate→up barrier
  - gate_tile_cnt = intermediate_tile_cnt (not //2)
  ...
  Phase 2: run the FC1 -> ReLU2 -> quant -> FC2 -> scatter datapath
           over the finalized expert-major packed input

The compute half:
  FC1:     A x w1^T                 (single FP4 block-scaled GEMM)
  ReLU2:   relu(fc1_out)^2          (squared ReLU activation)
...
class MoEStaticKernelRelu2:
    """Compact static MoE kernel (ReLU2 non-gated) for small routed working sets."""
    ...
    def _shared_storage_size_bytes(
        ...
        offset = (
            3 * 4
            + 2 * (self.ab_stage * 2 * 8)  # 2 pipelines (no up_pipeline)
            + _COMPACT_STATIC_TILE_M * 4
            + _COMPACT_STATIC_TILE_M * 4
        )
        buffers = (
            cute.size_in_bytes(self.a_dtype, a_smem_staged),   # sA
            cute.size_in_bytes(self.b_dtype, b_smem_staged),   # sB (FC1)
            cute.size_in_bytes(self.sf_dtype, sfa_smem_staged),  # sSFA
            cute.size_in_bytes(self.sf_dtype, sfb_smem_staged),  # sSFB
            cute.size_in_bytes(cutlass.BFloat16, epi_smem_staged),  # sC
        )
    ...
    def __call__(
        ...
        b_w13: cute.Tensor,            # [I_tp, K, E] — single FC1 weight (non-gated)
        ...
        # Single SF tensor for w13 scale factors
        ...
        # Single TMA descriptor over w13 [I_tp, K, E] (non-gated FC1 weight).
    ...
    def kernel(
        ...
        # Single tiled view over w13 [I_tp, K, E] (non-gated FC1 weight).
        ...
        # Single w13 TMA partition (non-gated: single FC1 weight)
        ...
        if warp_idx < self.num_mma_warps:
            ...
            while is_valid_tile:
                ...
                # ============================================================
                # PHASE A: FC1 for this slice (single GEMM, non-gated)
                # ============================================================

                # FC1 GEMM (inlined to avoid @cute.jit pass-by-value for acc)
                ...
                # ReLU2 + quant into sA (no up pass — non-gated activation)
                ...
                for epi_m in cutlass.range_constexpr(epi_rest_m):
                    ...
                    relu2_epi_buffer = Int32(epi_m) % cute.size(tRS_sD, mode=[3])
                    if epi_m_valid > Int32(0):
                        ...
                        cute.copy(
                            tiled_copy_r2s,
                            tRS_rD_out,
                            tRS_sD[(None, None, None, relu2_epi_buffer)],
                        )
                    ...
                    while quant_idx < epi_rows * sf_blocks_per_row:
                        ...
                        for elem_idx in cutlass.range_constexpr(16):
                            value = cutlass.Float32(
                                sC[local_row, block_start + elem_idx, relu2_epi_buffer]
                            )
                            values[elem_idx] = value
                            block_max = fmax_f32(block_max, fabs_f32(value))
                ...
                # No CTA-wide barrier needed here: FC1 is done with sB/sSFB
                # and DMA's B_down loads into sB/sSFB don't conflict with
                # MMA's ReLU2+quant on sC/sA/sSFA. The phase2_pipeline
        ...
        # ===================================================================
            ...
            while is_valid_tile:
                ...
                # W13 is a single FC1 weight (non-gated, no up/gate split).
                tBgB_w13_nk = tBgB_w13[(None, intermediate_slice, None, weight_expert_idx)]
                sfb_tile_coord = intermediate_slice // self.sfb_tiles_per_block
                tBgSFB_w13_nk = tBgSFB_w13[(None, sfb_tile_coord, None, weight_expert_idx)]

                # ---- FC1 pass (single GEMM, non-gated) ----
                ...
                for k_tile in range(0, fc1_k_tile_cnt, 1, unroll=4):
                    ...
                    cute.copy(tma_b_w13, tBgB_w13_nk[(None, k_tile)], tBsB_w13[(None, prod_state.index)], tma_bar_ptr=ml_pipeline.producer_get_barrier(prod_state))
                    ...
                    cute.copy(tma_sfb_w13, tBgSFB_w13_nk[(None, k_tile)], tBsSFB_w13[(None, prod_state.index)], tma_bar_ptr=ml_pipeline.producer_get_barrier(prod_state))
                    ml_pipeline.producer_commit(prod_state)
                    prod_state.advance()

                # ---- FC2 B_down loads: continuous pipeline ----
                # No barrier needed: sB/sSFB are free after FC1 completes.
