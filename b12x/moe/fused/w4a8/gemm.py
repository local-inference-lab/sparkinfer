...
class W4A8GemmKernel:
    ...
    def kernel(
        ...
        if cutlass.const_expr(self.experts_per_group):
            ...
            if e_grp >= Int32(0):
                ...
                self._run_tile(
                    a_q_flat, a_sf_flat, b_rp_flat, b_sf_flat, c_u32_flat,
                    packed_route_indices,
                    s_base, tid, mb0, e_grp, output_n_tile, active_m,
                    total_routes, en_mask, n_present,
                )
        else:
            ...
            if mismatch == Int32(0):
                if lead >= Int32(0):
                    ...
                    self._run_tile(
                        a_q_flat, a_sf_flat, b_rp_flat, b_sf_flat, c_u32_flat,
                        packed_route_indices,
                        s_base, tid, mb0, lead, output_n_tile, active_m,
                        total_routes, en_mask, n_present,
                    )
            else:
                ...
                for blk in cutlass.range_constexpr(_BLOCKS_PER_CTA):
                    ...
                    if e >= Int32(0):
                        self._run_tile(
                            a_q_flat, a_sf_flat, b_rp_flat, b_sf_flat,
                            c_u32_flat, packed_route_indices,
                            s_base, tid, mb0, e, output_n_tile, active_m,
                            total_routes, Int32(1 << blk), n_present,
                        )
    ...
    def _issue_stage(
        ...
        for blk in cutlass.range_constexpr(_BLOCKS_PER_CTA):
            if Int32(blk) < n_present:
                cp_async4_shared_global(
                    stage_base + Int32(blk * _A_BLOCK_WORDS * 4)
                    + (row << Int32(7)) + (p << Int32(4)),
                    get_ptr_as_int64(
                        a_q_flat, Int32(a_src_off[blk]) + kt * Int32(32)
                    ),
                )
    ...
    def _run_tile(
        ...
        self._issue_stage(
            a_q_flat, a_sf_flat, b_rp_flat, b_sf_flat,
            s_base, tid, Int32(0),
            a_src_off, asf_src_off, b_base, bsf_base,
            n_present,
        )
        ...
        while kt < k_tiles:
            ...
            for kb in cutlass.range_constexpr(4):
                ...
                a_lo = (
                    sa_base + (q << Int32(7)) + (u_phys << Int32(4))
                    + ((c & Int32(1)) << Int32(3))
                )
                a_frag = cute.make_rmem_tensor(
                    (_BLOCKS_PER_CTA, 4), cutlass.Uint32
                )
                for blk in cutlass.range_constexpr(_BLOCKS_PER_CTA):
        ...
        col_half_base = (
            output_n_tile * Int32(_TILE_N // 2) + warp * Int32(32) + c
        )
        ...
        for blk in cutlass.range_constexpr(_BLOCKS_PER_CTA):
            ...
            lim[blk] = blk_lim
        if cutlass.const_expr(self.gather_a):
            zero_row = cute.make_rmem_tensor(
                (_BLOCKS_PER_CTA, 2), cutlass.Int32
            )
            for blk in cutlass.range_constexpr(_BLOCKS_PER_CTA):
                z_lo = Int32(0)
                z_hi = Int32(0)
        ...
        for nt in cutlass.range_constexpr(_N8_PER_WARP):
            ...
            for blk in cutlass.range_constexpr(_BLOCKS_PER_CTA):
                p_lo = pack_f32x2_to_bfloat2(
                    facc[blk, nt, 0], facc[blk, nt, 1]
                )
                p_hi = pack_f32x2_to_bfloat2(
                    facc[blk, nt, 2], facc[blk, nt, 3]
                )
                if cutlass.const_expr(self.gather_a):
