...
This baseline keeps the existing compact prequantized workspace contract,
but removes the largest correctness-first bottlenecks from the original:
warp-cooperative sparse/shared routing, warp-cooperative FC1 prepack, and
metadata-only workspace clearing.
"""
...
def _append_compact_prequantized_row_warp(
    *,
    ...
    while sf_idx < sf_blocks_per_row:
        ...
        sf_idx += Int32(THREADS_PER_BLOCK)
...
class _MonolithicPreMLPKernel:
    ...
    def kernel(
        ...
        @cute.struct
        class Storage:
            norm_row: cute.struct.Align[
                cute.struct.MemRange[self.element_dtype, self.hidden_size],
                16,
            ]
        ...
        while col < Int32(self.hidden_size):
            ...
            out_cast = self.element_dtype(out)
            sNorm[col] = out_cast
            normalized_out[bidx, col] = out_cast
            col += Int32(THREADS_PER_BLOCK)
        cute.arch.sync_threads()
        ...
        while expert < Int32(self.num_sparse_experts):
            ...
            if tidx == Int32(0):
                candidate_val = cutlass.Float32(self.element_dtype(expert_acc))
                insert_slot = Int32(-1)
        ...
        for slot in cutlass.range_constexpr(self.combined_top_k):
            _append_compact_prequantized_row_warp(
                lane_id=lane_id,
                token_idx=bidx,
                expert_id=sSelectedIds[Int32(slot)],
                route_weight=sSelectedWeights[Int32(slot)],
