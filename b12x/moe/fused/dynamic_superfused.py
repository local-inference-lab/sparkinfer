"""
Queue-driven dynamic superfused MoE kernel for SM120.

This backend owns the full pre-MLP producer path and immediately publishes
ready FC1 tiles to the existing dynamic consumer body:

  producer:
    oneshot allreduce -> residual add -> Gemma RMSNorm -> routing/shared gate
    -> online tile allocation -> routed FP4/sfA writes -> ready-task publish

  consumer:
    ready-task pop -> FC1 -> SwiGLU -> FC2 -> scatter

The producer/consumer boundary is still the routed FC1-input FP4 payload in
gmem, but there is no full-grid route/pack -> compute barrier. Consumer CTAs
start working as soon as a tile is published.
"""
...
@cute.jit
def _append_dynamic_tile_row_warp(
    *,
    lane_id: Int32,
    token_idx: Int32,
    expert_id: Int32,
    ...
    if lane_id == Int32(0):
        ...
        tile_ordinal = row // Int32(_LEVEL_TILE_M)
        row_in_tile = row % Int32(_LEVEL_TILE_M)
        tile_slot = expert_id * max_tiles_per_expert + tile_ordinal
        tile_slot_addr = get_ptr_as_int64(expert_tile_ptr_flat, tile_slot)
        if row_in_tile == Int32(0):
            phys_tile = atomic_add_global_i32(
                get_ptr_as_int64(next_tile_alloc, Int32(0)),
                Int32(1),
            )
            _st_global_release_i32(tile_slot_addr, phys_tile)
        else:
            _spin_wait_global_eq_i32(tile_slot_addr, Int32(-1))
            phys_tile = _ld_global_acquire_i32(tile_slot_addr)
        phys_row = phys_tile * Int32(_LEVEL_TILE_M) + row_in_tile
        st_global_i32(get_ptr_as_int64(token_map, phys_row), token_idx)
        st_global_f32(get_ptr_as_int64(token_weights, phys_row), route_weight)

    row = cute.arch.shuffle_sync(row, Int32(0))
    phys_tile = cute.arch.shuffle_sync(phys_tile, Int32(0))
    row_in_tile = row % Int32(_LEVEL_TILE_M)

    if input_scales_are_reciprocal and gs_value != cutlass.Float32(0.0):
        if fast_math:
    ...
    while sf_idx < sf_blocks_per_row:
        ...
        output_offset = (
            (phys_tile * Int32(_LEVEL_TILE_M) + row_in_tile) * output_bytes_per_row
            + sf_idx * Int32(8)
        )
    ...
    _threadfence()
    cute.arch.sync_warp()
    return row, phys_tile, row_in_tile


class MoEDynamicSuperfusedKernel:
    """Queue-driven dynamic MoE kernel with an in-kernel pre-MLP producer."""
    ...
    def __call__(
        ...
        self.kernel(
            ...
            task_valid_rows, expert_tile_ptr_flat, tile_write_count, row_counts, max_tiles_per_expert, gate_tile_cnt,
            ...
            scatter_output, token_map, token_weights, eps,
        ).launch(
            grid=grid,
    ...
    def kernel(
        ...
        class Storage:
            ...
            scatter_tok_cache: cute.struct.MemRange[cutlass.Int32, _LEVEL_TILE_M]
            scatter_weight_cache: cute.struct.MemRange[cutlass.Float32, _LEVEL_TILE_M]
        ...
        route_gate_tile_cnt = gate_tile_cnt_runtime
        ...
        tile_slots = Int32(tile_write_count.shape[0])
        ...
        while i < num_experts:
            ...
            i += flat_stride
        i = flat_tid
        while i < expert_tile_ptr_flat.shape[0]:
            expert_tile_ptr_flat[i] = Int32(-1)
            i += flat_stride
        j = flat_tid
        ...
        while tw < tile_slots:
            ...
            tw += flat_stride
        if flat_tid == Int32(0):
            token_head[Int32(0)] = Int32(0)
            next_tile_alloc[Int32(0)] = Int32(0)
        ...
        while produce_active > Int32(0):
            token_idx = Int32(0)
            if is_cta_leader > Int32(0):
                token_idx = atomic_add_global_i32(
                    get_ptr_as_int64(token_head, Int32(0)),
                    Int32(1),
                )
                _st_shared_i32(ctrl_base_addr + Int32(28), token_idx)
            cute.arch.sync_threads()
            token_idx = _ld_shared_i32(ctrl_base_addr + Int32(28))
            if token_idx >= num_tokens:
                ...
                if warp_idx == Int32(_SUPERFUSED_NORM_WARP_IDX):
                    ...
                    while col < cols:
                        ...
                        sNorm[col] = cutlass.BFloat16(out)
                        col += Int32(_WARP_SIZE)
                cute.arch.sync_threads()
                    ...
                    if lane_id == Int32(0):
                        ...
                        if self.renormalize_topk:
                            ...
                            for slot in cutlass.range_constexpr(self.top_k):
                                ...
                                sSelectedWeights[Int32(slot)] = exp_vals[slot] * inv_denom
                        else:
                            for slot in cutlass.range_constexpr(self.top_k):
                                sSelectedIds[Int32(slot)] = top_ids[slot]
                                sSelectedWeights[Int32(slot)] = top_vals[slot]

                        sSelectedIds[Int32(self.top_k)] = Int32(self.num_sparse_experts)
                        sSelectedWeights[Int32(self.top_k)] = shared_gate
                cute.arch.sync_threads()
                ...
                if warp_idx >= Int32(_SUPERFUSED_PACK_WARP_BASE):
                    ...
                    while slot < Int32(self.combined_top_k):
                        ...
                        row, phys_tile, row_in_tile = _append_dynamic_tile_row_warp(
                            lane_id=lane_id,
                            token_idx=token_idx,
                            expert_id=expert_id,
                            ...
                            expert_tile_ptr_flat=expert_tile_ptr_flat,
                            next_tile_alloc=next_tile_alloc,
                            ...
                            input_scales_are_reciprocal=self.input_scales_are_reciprocal,
        ...
        if is_cta_leader > Int32(0):
            ...
            if prev_done == Int32(gdim_z) - Int32(1):
                ...
                while expert_flush < num_experts:
                    ...
                    rem = rows % Int32(_LEVEL_TILE_M)
                    if rem != Int32(0):
                        tile_ordinal = rows // Int32(_LEVEL_TILE_M)
                        tile_slot = expert_flush * max_tiles_per_expert + tile_ordinal
                        phys_tile = _ld_global_acquire_i32(
                            get_ptr_as_int64(expert_tile_ptr_flat, tile_slot)
                        )
                        self._publish_ready_tasks(
                            task_tail, task_ready,
                            task_expert, task_m_tile,
...
__all__ = ["MoEDynamicSuperfusedKernel"]
