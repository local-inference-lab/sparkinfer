...
def main() -> None:
    ...
    spec = _make_spec()
    m = 2
    hidden_local = make_input_activations(spec, m, seed=7100 + _rank(), device=device)
    residual = make_input_activations(spec, m, seed=8100 + _rank(), device=device)
    ...
    try:
        ...
        fused_out, fused_residual_out = _b12x_gemma_moe_block_fp4_static_monolithic(
            hidden_local,
            residual,
