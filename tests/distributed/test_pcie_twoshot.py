...
def _check_reduce_scatter(pool: PCIeTwoShotSP, rank: int, world: int, step: int) -> None:
    device = pool.device
    payloads = []
    scales = []
...
def _check_graph_capture(pool: PCIeTwoShotSP, rank: int, world: int) -> None:
    ...
    ag_q = torch.zeros(rows_per_rank, ROW_ELEMS, dtype=torch.float8_e4m3fn, device=device)
...
def _bench(pool: PCIeTwoShotSP, rank: int, world: int) -> None:
    ...
    rs_out = torch.empty(rows_per_rank, ROW_ELEMS, dtype=torch.bfloat16, device=device)
    ag_out = torch.empty(ROWS, ROW_ELEMS, dtype=torch.bfloat16, device=device)
    nccl_rs_out = torch.empty(rows_per_rank, ROW_ELEMS, dtype=torch.bfloat16, device=device)
    ...
    if rank == 0:
        ...
        print(f"[{world} ranks, {ROWS}x{ROW_ELEMS}, payload {payload_mb:.0f} MB bf16-equiv]")
