...
def _apply_memory_debug_patch() -> None:
    ...
    if not hasattr(cuda_helpers, "_memory_debug_snapshot") or not hasattr(
        ...
        return
    if getattr(cuda_helpers, "_sparkinfer_memory_debug_patched", False):
        _MEMORY_DEBUG_PATCHED = True
        return

    if not hasattr(cuda_helpers, "_sparkinfer_original_memory_debug_snapshot"):
        cuda_helpers._sparkinfer_original_memory_debug_snapshot = (
            cuda_helpers._memory_debug_snapshot
        )
    if not hasattr(cuda_helpers, "_sparkinfer_original_memory_debug_log"):
        cuda_helpers._sparkinfer_original_memory_debug_log = cuda_helpers._memory_debug_log
    ...
    cuda_helpers._sparkinfer_memory_debug_patched = True
    _MEMORY_DEBUG_PATCHED = True
...
class _DirectFrameInfoInspectProxy:
    ...
    _sparkinfer_direct_frameinfo = True
...
def _apply_direct_frameinfo_patch() -> None:
    ...
    if getattr(current_inspect, "_sparkinfer_direct_frameinfo", False):
