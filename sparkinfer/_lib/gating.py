"""Capability gating for sparkinfer.
...
def is_sparkinfer(device=None) -> bool:
...
def require_cutlass_dsl(minimum: str = MIN_CUTLASS_DSL) -> None:
    ...
    if version is None:
        raise ImportError(
            "sparkinfer requires nvidia-cutlass-dsl "
    ...
    if _version_tuple(version) < _version_tuple(minimum):
        raise ImportError(
            f"sparkinfer requires nvidia-cutlass-dsl >= {minimum}, "
...
def require_triton(op: str) -> None:
    if not has_triton():
        raise ImportError(
            f"sparkinfer.{op} requires triton (bundled with "
