...
``sparkinfer/attention/indexer/kernel.py`` (``_compute_mxfp8_tile_partials`` et al.)
and the radix select / virtual-index loaders from
``sparkinfer/attention/indexer/tiled_topk.py``; nothing is reinvented here.
"""
...
class SPARKINFERFusedIndexerScratchCaps:
...
def plan_fused_indexer_scratch(
    caps: SPARKINFERFusedIndexerScratchCaps,
) -> SPARKINFERFusedIndexerScratchPlan:
    ...
    return SPARKINFERFusedIndexerScratchPlan(
        caps=caps,
...
def _launch_fused(kernel, cute_args, key_tensors, policy):
    """Graph-safe launch via sparkinfer_launch (compile-once + replayable run_compiled).

    Unlike a bare cute.compile()+call, the sparkinfer_launch path is CUDA-graph-capturable
