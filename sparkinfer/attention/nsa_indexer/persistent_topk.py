...
class SPARKINFERPersistentTopK2048ScratchCaps:
...
@dataclass(frozen=True, kw_only=True)
class SPARKINFERPersistentTopK2048Binding:
    logits: torch.Tensor
...
def plan_persistent_topk2048_scratch(
    caps: SPARKINFERPersistentTopK2048ScratchCaps,
) -> SPARKINFERPersistentTopK2048ScratchPlan:
    scratch_nbytes = _persistent_topk2048_capacity_nbytes(
        caps.max_rows,
        caps.max_stride,
        device=caps.device,
    )
    return SPARKINFERPersistentTopK2048ScratchPlan(
        caps=caps,
...
def build_persistent_topk2048_binding(
    *,
    ...
    return SPARKINFERPersistentTopK2048Binding(
        logits=logits,
        lengths=lengths,
        scratch=scratch,
        page_table_1=page_table_1,
...
def run_persistent_topk2048(
    ...
    *,
    ...
    max_seq_len: int | None = None,
    topk: int = _TOPK,
    binding: SPARKINFERPersistentTopK2048Binding | None = None,
) -> torch.Tensor:
