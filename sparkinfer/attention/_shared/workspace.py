...
SPARKINFERWorkspaceMode = Literal["decode", "extend", "verify", "draft_extend"]


@dataclass(frozen=True, kw_only=True)
class SPARKINFERAttentionArenaCaps:
    device: torch.device
    dtype: torch.dtype
...
@dataclass(frozen=True, kw_only=True)
class SPARKINFERAttentionWorkspaceContract:
    mode: SPARKINFERWorkspaceMode
    max_total_q: int
    max_batch: int
...
@dataclass(frozen=True, kw_only=True)
class _SPARKINFERAttentionArenaLayout:
    arena_nbytes: int
    ...
    mhc_comb_nbytes: int
    mhc_out_nbytes: int
    wo_projection_layout: _SPARKINFERWOProjectionArenaLayout
...
class SPARKINFERAttentionArena:
    caps: SPARKINFERAttentionArenaCaps
    ...
    def _layout(cls, caps: SPARKINFERAttentionArenaCaps) -> _SPARKINFERAttentionArenaLayout:
        ...
        return _SPARKINFERAttentionArenaLayout(
    ...
    @classmethod
    def allocate(cls, caps: SPARKINFERAttentionArenaCaps) -> "SPARKINFERAttentionArena":
    ...
    @classmethod
    def required_nbytes(cls, caps: SPARKINFERAttentionArenaCaps) -> int:
    ...
    def _make_workspace_views(
        self,
        contract: SPARKINFERAttentionWorkspaceContract,
        *,
        ...
        workspace = SPARKINFERAttentionWorkspace(
            arena=self,
            contract=contract,
            mode=contract.mode,
...
@dataclass(kw_only=True)
class SPARKINFERAttentionWorkspace:
    arena: SPARKINFERAttentionArena | None = None
    contract: SPARKINFERAttentionWorkspaceContract | None = None
    mode: SPARKINFERWorkspaceMode
    device: torch.device
    dtype: torch.dtype
    kv_dtype: torch.dtype
    ...
    def for_fixed_capacity(
        ...
        *,
        ...
        arena = SPARKINFERAttentionArena.allocate(caps)
        contract = SPARKINFERAttentionWorkspaceContract(
            mode=mode,
            max_total_q=max_total_q,
            max_batch=max_batch,
    ...
    def _reserve_fused_indexer_scratch(self) -> None:
        ...
        if self._sparkinfer_fused_indexer_scratch is not None:
            return
        ...
        self._sparkinfer_fused_indexer_scratch = (
    ...
    def get_fused_indexer_scratch(
        ...
        cache = self._sparkinfer_fused_indexer_scratch
        if cache is None:
            raise RuntimeError(
