...
class MXFP8LinearWeight:
    """ModelOpt-style MXFP8 linear weight packed for sparkinfer dense GEMM."""
...
def _c_dtype_name(dtype: torch.dtype) -> str:
    ...
    if dtype == torch.float16:
        return "float16"
    raise ValueError(f"sparkinfer MXFP8 linear output dtype must be bf16/fp16, got {dtype}")
...
def pack_mxfp8_linear_weight(
    ...
    ModelOpt MXFP8 checkpoints require only `K % 32 == 0`. The underlying sparkinfer
...
def mxfp8_linear(
    ...
    *,
    ...
    """Run a ModelOpt MXFP8 linear through the native sparkinfer dense GEMM path."""
    ...
    if tokens == 0:
        ...
        output = torch.ops.sparkinfer.mxfp8_linear_fused(
            source_2d,
