...
same operation torch performs.  NOTE: the existing fp4-path dequant
primitives (packed_dequant_e4m3x4_to_bfloat2x2 / packed_dequant_e8m0x4_to_
bfloat2x2 in sparkinfer/_lib/intrinsics.py) are *biased* variants (scale-only sign
...
Torch custom ops (with fake impls for torch.compile tracing):
    torch.ops.sparkinfer.qbmm_absorb_ukt(a, pack_values, pack_scales, out, stream_int)
    torch.ops.sparkinfer.qbmm_absorb_uv (a, pack_values, pack_scales, out, stream_int)
...
from sparkinfer._lib.utils import current_cuda_stream, get_num_sm, make_ptr
...
# ---------------------------------------------------------------------------
# Torch custom ops (op + fake pattern, per sparkinfer::dense_gemm_launch /
# sparkinfer::mxfp8_linear_fused).  out-mutating, None-returning: safe for
# CUDA-graph capture and torch.compile tracing.
# ---------------------------------------------------------------------------


@torch.library.custom_op("sparkinfer::qbmm_absorb_ukt", mutates_args=("out",))
...
@torch.library.custom_op("sparkinfer::qbmm_absorb_uv", mutates_args=("out",))
...
# the mxfp8-linear serving warmup / the aiter per-batch-size precompile loop.
# ---------------------------------------------------------------------------
