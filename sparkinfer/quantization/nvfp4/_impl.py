...
from sparkinfer.cute.utils import current_cuda_stream, get_max_active_clusters, get_num_sm, make_ptr
from sparkinfer.quantization.bf16_to_fp4_tma import Bf16ToFp4TmaKernel
...
def _validate_tiled_shape(M: int, K: int) -> None:
    ...
    if M % _TILE_M != 0 or K % _TILE_K != 0:
        raise ValueError(
            f"M and K must be multiples of ({_TILE_M}, {_TILE_K}), "
            f"got M={M}, K={K}"
        )
...
def compile_bf16_to_fp4_tma(M: int, K: int):
    ...
    scale_fake = cute.runtime.make_fake_compact_tensor(
        cutlass.Uint8, (M * K // _SF_VEC_SIZE,), assumed_align=16,
    )
    ...
    def launch(bf16_input, global_scale, packed_a_flat, scale_flat):
        ...
        if _overlaps(global_scale, packed_a_flat) or _overlaps(global_scale, scale_flat):
