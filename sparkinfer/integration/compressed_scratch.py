...
class SPARKINFERCompressedMLAScratchCaps:
...
@dataclass(frozen=True, kw_only=True)
class _SPARKINFERCompressedMLAScratchLayout:
    nbytes: int
    max_q_chunks: int
...
class SPARKINFERCompressedMLAScratch:
...
def _compressed_mla_scratch_layout(
...
) -> _SPARKINFERCompressedMLAScratchLayout:
    ...
    return _SPARKINFERCompressedMLAScratchLayout(
...
def _install_compressed_mla_contract_phantoms(
    scratch: SPARKINFERCompressedMLAScratch,
) -> None:
...
def plan_compressed_mla_scratch(
    ...
    return SPARKINFERCompressedMLAScratchPlan(
        caps=caps,
        layout=layout,
        _scratch_specs=(
            scratch_buffer_spec(
