...
plain SPARKINFERSparseMLAScratch views container (mirroring SPARKINFERCompressedMLAScratch).
It never constructs a SPARKINFERAttentionWorkspace / arena, allocates, or init-writes.
...
class SPARKINFERSparseMLAScratchCaps:
...
class SPARKINFERSparseMLAScratch:
    ...
    kernels duck-type off the (former) workspace. NEVER a SPARKINFERAttentionWorkspace.
    """
...
@dataclass(frozen=True, kw_only=True)
class SPARKINFERSparseMLABinding:
    scratch: object
    q: torch.Tensor
    selected_indices: torch.Tensor
    cache_seqlens_int32: torch.Tensor
...
def build_sparse_mla_binding(
    *,
    ...
    return SPARKINFERSparseMLABinding(
        scratch=scratch,
        q=q,
        selected_indices=selected_indices,
        cache_seqlens_int32=cache_seqlens_int32,
...
class _SPARKINFERSparseMLAScratchLayout:
    nbytes: int
    split: bool
...
def _sparse_mla_scratch_layout(
...
) -> _SPARKINFERSparseMLAScratchLayout:
    ...
    return _SPARKINFERSparseMLAScratchLayout(
...
def _materialize_sparse_mla_scratch(
    ...
    scratch = SPARKINFERSparseMLAScratch(
...
def plan_sparse_mla_scratch(
    ...
    return SPARKINFERSparseMLAScratchPlan(
        caps=caps,
        layout=layout,
        _scratch_specs=(
            scratch_buffer_spec(
